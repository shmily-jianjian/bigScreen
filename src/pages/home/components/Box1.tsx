...
const NumItem: React.FC<ItemProps> = ({number}) => {
  ...
  return <div className={styles.numItem}>
    {
      numberArr.map((item, index) => (
        ...
        }} className={styles.item} key={index}>{item}</div>
      ))
    }
  </div>
}
...
const Box1 = () => {
  const [data, setData] = useState<string>((new Date().getMilliseconds() + Math.floor(Math.random() * 99999)).toString())
  const newData = data.toString().split('')

  const changeData = () => {
    setData((new Date().getMilliseconds() + Math.floor(Math.random() * 99999)).toString())
  }
